...
coverage_type_info_clientArray.forEach(coverage_type_info_clien => {
  file1.forEach($car => {
    ...
    it.skip(`toni_automotive_self_service execute vin ${$car[0]}`, () => {
      cy.createNewQuestionnaire('toni_automotive_self_service',) //toni_automotive_self_service, toni_automotive_claim_handler

      cy.wait(2000)
      let requestUrl = Cypress.env('requestUrl')
      //requestUrl = 'https://dev03.spearhead-ag.ch:443/p/r/0qdsWLX31n85MmsFP0hL5'
      if(requestUrl == undefined || requestUrl == null || !requestUrl.length > 0){
        throw new Error(`test fails : requestUrl = ${requestUrl}`)
      }
      console.log(`Start ${Cypress.env('templateId')} from url: ${requestUrl}.`)
      ...
      const questionnaireId = Cypress.env('questionnaireId')
      //const questionnaireId = 'GDykGMDYU5zbevquRuZiY'
      ...
      cy.get('@goingPageId').then(function (aliasValue) {
        if (aliasValue == 'page-05'){
          ...
          cy.getQuestionAnswer('coverage-type-info').then(function (answer) {
            console.log(`coverage-type-info : ${answer}`);
            const coverage_type_arr = new Array(
              'glass',
              'OptionalParkCoverageParkingDamage',
              'PartiallyComprehensiveMarten',
              'OptionalAssistance',
              'OptionalLegalAssistance'
            )
            if (coverage_type_arr.includes(answer.toString())){
              cy.get('input#incident-date-input').type(`01-06-2024`)
            }
            const coverage_type_arr2 = new Array(
              'VehicleLiabilityNotCustomer',
              'collision',
              'PartiallyComprehensiveAnimalCollisionDamageCarIH',
              'natural-disaster',
              'PartiallyComprehensiveFire',
              'theft-robbery-embezzlement',
              'PartiallyComprehensiveVandalism')
      ...
      cy.get('@goingPageId').then(function (aliasValue) {
        if (aliasValue == 'page-09'){
         // "coverage-type-info": ["natural-disaster","PartiallyComprehensiveFire"]
          nextBtn()
        }
      })
      ...
      cy.get('@goingPageId').then(function (aliasValue) {
        if (aliasValue == 'page-17'){
          ...
          cy.get(`div#${compoundQuestion}`).find(`input#${compoundQuestion}-first-name__--__0-input`).type(`first name`)
          cy.get(`div#${compoundQuestion}`).find(`input#${compoundQuestion}-last-name__--__0-input`).type(`last name`)
          cy.get(`div#${compoundQuestion}`).find(`input#${compoundQuestion}-street-name__--__0-input`).type(`Street name`)
          cy.get(`div#${compoundQuestion}`).find(`input#${compoundQuestion}-street-number__--__0-input`).type(`123A-1`)
          cy.get(`div#${compoundQuestion}`).find(`input#${compoundQuestion}-zip-code__--__0-input`).type(`8000`)
          //owner-city
          cy.get(`div#${compoundQuestion}`).find(`input#${compoundQuestion}-phone-number__--__0-input`).type(`1234567890`,{delay : 200})
          cy.get(`div#${compoundQuestion}`).find(`input#${compoundQuestion}-email__--__0-input`).type(`sivanchevski@soft2run.com`)
          cy.get(`div#${compoundQuestion}`).find(`input#${compoundQuestion}-vehicle-license-plate__--__0-input`).type(`TONI 91911`)
          cy.get(`div#${compoundQuestion}`).find(`input#${compoundQuestion}-vehicle-brand__--__0-input`).type(`vehicle-brand`)
          cy.get(`div#${compoundQuestion}`).find(`input#${compoundQuestion}-vehicle-description__--__0-input`).type(`vehicle-description`)
          cy.get(`div#${compoundQuestion}`).find(`input#${compoundQuestion}-vehicle-insurance__--__0-input`).type(`vehicle-insurance`)
          cy.get(`div#${compoundQuestion}`).find(`textarea#${compoundQuestion}-damage-description__--__0-textarea`).type(`Bitte beschreiben Sie die Beschädigung am Fahrzeug des Geschädigten:`)
          nextBtn()
        }
      })
