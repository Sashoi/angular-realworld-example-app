...
describe('Start and complete Vehicle_Zone questionnaire'
  ...
  , () =>{
    ...
    it('Get token', () =>{
      ...
      cy.request('POST',`https://${$dev}.spearhead-ag.ch/member/authenticate`,userCredentials).then(
        (response) => {
    ...
    it('Create self-service-init questionnaire', () =>{
      ...
      const options = {
            method: 'POST',
            url: `https://${$dev}.spearhead-ag.ch:443/b2b/integration/huk/huk-comprehensive-self-service-init`,
            body: b2bBody,
            headers: {
              'Accept': '*/*',
    ...
    it('Get damageNotificationId', () =>{
      ...
      const options = {
        method: 'GET',
        url: `https://${$dev}.spearhead-ag.ch:443/questionnaire/${Cypress.env("questionnaireId")}`,
        headers: {
          'Accept': '*/*',
          'Accept-Encoding':'gzip, deflate, br',
          'Content-Type': 'application/json',
    ...
    it('From DN get questionnaire Vehicle_Zone url', () =>{
        ...
        const options = {
          method: 'GET',
          url: `https://${$dev}.spearhead-ag.ch:443/damage/notification/${Cypress.env("damageNotificationId")}`,
          headers: {
            'Accept': '*/*',
            'Accept-Encoding':'gzip, deflate, br',
            'Content-Type': 'application/json',
        ...
        cy.request(options).then(
          (response) => {
            ...
            cy.task('setQurl', questionnaireUrl)
          }
        )
      }
    )
    ...
    it('Start questionnaire Vehicle_Zone and fulfill page-01', () =>{
      cy.intercept('GET', `https://${$dev}.spearhead-ag.ch/questionnaire/*/currentPage?offset=120&locale=de`).as('currentPage')
      cy.wait(4000)
      ...
      cy.intercept('POST', `https://${$dev}.spearhead-ag.ch/questionnaire/*/page/page-01?navigateTo=next&offset=120&locale=de`).as('navigateTo')
    ...
    it('Fulfill page-08', () =>{
      ...
      cy.wait(10000);
      cy.get('div[class^="error-container"]').should('not.exist');
    ...
    it('Fulfill page-09', () =>{
      ...
      cy.wait('@currentPage').then(xhr => {
        ...
        const pageId = xhr.response.body.pageId;
        console.log(pageId)
        expect(pageId).to.eq('page-09') // true
      })
